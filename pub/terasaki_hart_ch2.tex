...
using univariate models of the selective environment
...
leading to the emergence of novel physical landscapes \cite{williams_novel_climates,williams_projected_novel_disappearing,fitzpatrick_climate_novelty_forecasts}
and, consequently, novel adpative landscapes (see \ref{fig:conceptual}).
...
is adaptive or maladaptive, and by how much) may depend 
...
whether adaptation to climate change will happen at all.
...
in multivariate selective environments under climate change \cite{capblancq_review}.
...
of bivariate trait space (decpited in \ref{fig:conceptual}).
...
We run 100 pairs of main and null (i.e., stable-climate) simulations, for each of eighteen scenarios
resulting from the full factorial crossing of:
\begin{enumerate}
    \item two levels of redundancy (moderate: mid-range phenotypes can result from numerous genotypes but extreme phenotypes are the result of unique, extreme genotypes; and high in which all phenotypes can result from numerous genotypes; see \ref{fig:SUPP_redundancy});
    \item three levels of polygenicity (low: 4 or 8 loci per trait, depending on the level of redundancy; moderate: 20 or 40 loci; and high: 100 or 200 loci);
...
While our results generally support the first-order assumption underlying AGF,
our framework also highlights a broader area of important applied research: 
If landscape genetics can cast 
...
Under moderate redundancy, mean fitness declined during climate change across all scenarios, (GIVE ANOVA 
RESULTS; see right left half of Figure \ref{fig:fit_over_time_paneled}, as did population size (GIVE ANOVA 
RESULTS; see left half of Figure S1). The extent of decline was 
positively correlated with the strength of linkage, (GIVE HSD RESULTS), and 
displayed a non-linear relationship with polygenicity. High-polygenicity (i.e., 100-locus) 
scenarios showed dramatic declines (HSD RESULTS), with mean fitness in the strong-linkage scenario
declining by XXXXXX\% on average (from XXX to XXX),
and mean population size decliining by 19\% on average (from ~10,500 to ~8000 individuals).
Notably, these declines were also uniquely persistent,
with no evidence of evolutionary rescue (i.e., no stabilization and rebound occurring during climate change), 
...
All scenarios displayed some maladaptation, as evidenced by the red wedges displayed
in the 'after' scatterplots of Figure \ref{fig:pheno_shift}, for moderate redundancy,
and Figure S2, for high redundancy.
However, maladaptation was significantly higher in the moderate-redundancy, high-polygenicity scenarios,
...
(as well as the before-after comparisons of moderate-redundnacy population density plotted in
...
(GIVE ANOVA AND HSD RESULTS; see Figure \ref{fig:gene_flow} for moderate-redundancy results
and Figure S4 for equivalent high-redundancy results).
Against this background expectation, up-gradient gene flow
was greater in main than null simulations for all scenarios.
The scenarios with the strongest on-contour gene flow (strong-linkage with moderate-polygenicity,
...
FROM IAN CONVO NOTES:
 ...
 4. possible that 20-locus min-effect result is an artefact or a biologically significant finding, but more work required to better understand this
...
moderate-redundancy, high-polygenicity scenarios,
...
To the contrary, the moderate-redundancy, high-polygenicity scenarios,
...
that occurred in the moderate-redundancy, high-polygenicity scenarios
...
(the \texttt{n\_loci} values specified above provide moderate redundancy that drops
to a single genotype underlying each of the phenotypes matching extreme environmental
values of 0 and 1, whereas doubling those values provides significant redundancy across
all phenotypes 0 to 1; see Figure SXXXX).
