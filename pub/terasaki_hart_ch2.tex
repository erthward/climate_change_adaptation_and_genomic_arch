...
One component of genetic architecture is the number of loci 
...
A second component of genetic architecture, the degree of linkage between underlying loci
...
A third component of genetic architecture is
genetic redundancy \cite{barghi_polygenic,laruson},
which describes the number of different genotypes
that can produce identical phenotypes \cite{yeaman_review,laruson,barghi_polygenic}.
...
underlain by transient genetic architectures, the result
...
change.) We run many pairs of main and null simulations for each of eighteen scenarios, 
resulting from the full factorial crossing of three levels of linkage (no linkage, weak 
linkage, strong linkage), three values for the number of genes underlying each 
trait (4, 20, 100), and two values of redundancy
(moderate, in which mid-range phenotypic values
can be generated by numerous distinct genotypes while extreme phenotypic
values can be produced by single, extreme genotypes; and high,
in which all phenotypes can result from numerous genotypes). 
...
\begin{enumerate}
    ...
    \item High redundancy facilitates adaptation, resulting in smaller reductions in population size and mean fitness than in moderate-redundancy scenarios.
    \item Greater redundancy increases the ability of populations to adapt \textit{in situ} by shifting allelic covariance, resulting in less pronouned up-gradient gene flow than in moderate-redundancy scenarios.
\end{enumerate}


Our analysis provides important theoretical developments
in a neglected area of research,
confirming that some principles derived from models of static environments
also apply under environmental change, while simultaneously
revealing dynamics that would not be evident on the basis of
