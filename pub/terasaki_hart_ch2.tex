...
environmental gradients decouple,
generating novel multivariate 
environments that stress wild populations.
A commonly assumed mechanism of evolutionary rescue is adaptive gene flow
from populations whose current climates
approximate future projections --- 
but novel multivariate environments have no current analogs by definition,
and gene flow from similar climates can cause maladaptive introgression with respect to decoupling environmental variables.
Genomic architecture can play an important but often-ignored role in
determining the effectiveness and relative magnitudes of adaptive gene flow and \textit{in situ} adaptation.
Here, we use \textit{Geonomics}, a Python \cite{rossum} package
...
under scenarios of variable trait polygenicity, linkage, and genotypic redundancy.
We test a series of hypotheses
about cross-scenario variability in evolutionary outcomes,
reporting results that partially corroborate but also complicate or extend previous findings based on univariate and static environments.
First, we find that up-gradient gene flow is consistently present,
but its magnitude is strongly constrained under lower linkage and higher polygenicity and redundancy,
suggesting \textit{in situ} adaptation
as a more effective mechanism of evolutionary rescue under these conditions.
Second, we find that high polygenicity
causes increased maladaptation and acute
demographic decline, a concerning result given the likely polygenic nature
of many climate-adapted traits.
Finally, we show that adaptive capacity increases under 
...
Climate change makes climate gradients
shift in space and decouple,
causing the emergence of novel environments.
Using that conceptual basis,
our simulation results reveal
the complex and previously unrecognized dependence of
adaptation to climate change on the
genomic architectures of the traits involved:
the number of loci influencing a trait ('polygenicity'),
how frequently the loci are recombined in offspring
('linkage'), and the number of different genotypes that
yield identical phenotypes ('genotypic redundancy').
We show that \textit{in situ} adaptation
can be more effective than
climate-tracking adaptive gene flow under certain architectures;
and that high polygenicity can cause
local populations to decline,
but that high redundancy ameliorates this
and generally improves adaptive capacity.
}
