...
In nature, species are simultaneously adapted to multiple environmental gradients. When climate change shifts gradients at different rates and in different directions, populations experience novel selective pressures. Understanding evolutionary responses to these changes is essential for preserving biodiversity under climate change. Here, we use individual-based, spatially-explicit simulations to examine how genomic architecture can influence the efficacy of gene flow and adaptation for species persistence under multivariate environmental change. Our analyses reveal that linkage, polygenicity, and genotypic redundancy all contribute to determining the efficacy of adaptation and to influencing the relative contributions of climate-tracking gene flow and \textit{in situ} selection to that adaptation. These findings provide new insights into the role that genomic architecture plays in adaptation to climate change.
}
...
in mean fitness (Fig. \ref{fig:fig_2}) or population size (Fig. \ref{fig:fig_s2}), aside from small modeling artefacts that applied to both the 
...
that would optimally track the changing environment (Figs. \ref{fig:fig_4} and \ref{fig:fig_s3}), associated with increasing linkage (maladaptation: $\beta_{l} = 0.0038\pm0.0004, P<1\times10^{-15}$).
...
and highest at high polygenicity and low redundancy (Figs. \ref{fig:fig_3} and \ref{fig:fig_s2}).
...
(i.e., stabilization and rebound) occurring until the post-change period (Figs. \ref{fig:fig_3} and \ref{fig:fig_s2}).
...
genotypic redundancy can facilitate adaptation to shifting environmental gradients (Figs. \ref{fig:fig_2} and \ref{fig:fig_s2}).
...
of the low polygenicity scenarios (Figs. \ref{fig:fig_3} and \ref{fig:fig_s2}).
...
each of our 18 scenarios, combining the results for all 100 iterations under each scenario. For each time series we calculated the mean and 5th and 95th percentiles at each time step (Figs. \ref{fig:fig_s2} and \ref{fig:fig_3}).
...
We also produce these plots (Fig. \ref{fig:fig_s3}) using data from our null simulations to demonstrate that all differences
...
% reset table and figure counters and add 'S' to names
\setcounter{table}{0}
\makeatletter 
